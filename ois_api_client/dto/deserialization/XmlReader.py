...
class XmlReader:
    ...
    @staticmethod
    def get_child_int(parent: ET.Element, tag_name: str, namespace: str = NAMESPACE_API) -> Optional[int]:
    ...
    @staticmethod
    def get_child_float(parent: ET.Element, tag_name: str, namespace: str = NAMESPACE_API) -> Optional[float]:
        text = XmlReader.get_child_text(parent, tag_name, namespace)
        return None if text is None else float(text)

    @staticmethod
    def get_child_date(parent: ET.Element, tag_name: str, namespace: str = NAMESPACE_API) -> Optional[date]:
        text = XmlReader.get_child_text(parent, tag_name, namespace)
        return None if text is None else datetime.strptime(text, '%Y-%m-%d')

    @staticmethod
    def get_child_utc_datetime(parent: ET.Element, tag_name: str, namespace: str = NAMESPACE_API) -> Optional[datetime]:
        text = XmlReader.get_child_text(parent, tag_name, namespace)
